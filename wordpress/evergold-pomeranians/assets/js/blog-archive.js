(function () {
  ...
  const TOTAL = 5000;
  const PAGE_SIZE = 24;
  const listUrl = (window.evergoldBlog && evergoldBlog.list) || "blog.html";

  function listHref(params) {
    const url = new URL(listUrl, window.location.href);
    Object.keys(params).forEach(function (key) {
      if (params[key] === null || params[key] === "" || params[key] === 1 && key === "archive") {
  ...
  function renderArchive() {
    ...
    const page = Math.max(1, Math.min(Math.ceil(TOTAL / PAGE_SIZE), parseInt(params.get("archive") || params.get("page") || "1", 10) || 1));
    ...
    if (page > 1) {
      buttons.push('<a href="' + listHref({ archive: page - 1 }) + '">Prev</a>');
    }
    if (windowStart > 1) {
      buttons.push('<a href="' + listHref({ archive: 1 }) + '">1</a>');
    }
    for (let p = windowStart; p <= windowEnd; p += 1) {
      if (p === page) {
        buttons.push('<button class="is-current" type="button">' + p + "</button>");
      } else {
        buttons.push('<a href="' + listHref({ archive: p }) + '">' + p + "</a>");
      }
    }
    if (windowEnd < pages) {
      buttons.push('<a href="' + listHref({ archive: pages }) + '">' + pages + "</a>");
    }
    if (page < pages) {
      buttons.push('<a href="' + listHref({ archive: page + 1 }) + '">Next</a>');
    }
  ...
  function renderEntry() {
    ...
    const id = parseInt(new URLSearchParams(window.location.search).get("card") || new URLSearchParams(window.location.search).get("id") || "1", 10);
